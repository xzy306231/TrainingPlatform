...
{
    public object UpdateStuScore(List<ExamResult> examResults)
    {
        try
        {
                ...
                if (examResults != null && examResults.Count > 0)
                {
                    ...
                    for (int i = 0; i < examResults.Count; i++)
                    {
                        var query = from d in db.t_learning_record
                                    where d.delete_flag == 0 && d.content_id == queryContentF.id && d.user_id == examResults[i].UserNumber
                                    select d;
                        if (query.FirstOrDefault() == null)//不存在
                        {
                            t_learning_record r = new t_learning_record();
                            r.content_id = queryContentF.id;
                            r.user_id = examResults[i].UserNumber;
    ...
    public object UpdatePlanProgress(List<PlanProgress> planProgresses)
    {
        try
        {
            using (var db = new pf_course_manageContext())
    ...
    public object UpdateExamFinishRate(ExamFinishRate examFinishRate)
    {
        try
        {
            using (var db = new pf_course_manageContext())
