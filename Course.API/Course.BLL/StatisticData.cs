...
namespace Course.BLL
{
    public class StatisticData
    {
        ...
        /// </summary>
        /// <returns></returns>
        public object GetCurrentTrainingPlanCount()
        {
            try
            {
        ...
        /// </summary>
        /// <returns></returns>
        public object GetTrainingPlanInfo()
        {
            try
            {
                using (var db = new pf_course_manageContext())
        ...
        /// <returns></returns>
        public object GetTrainingPlanInfoByYear(int year)
        {
            try
            {
                using (var db = new pf_course_manageContext())
        ...
        /// </summary>
        /// <returns></returns>
        public object GetTrainingPlanFinishRate(bool IsAsc, string FeildName)
        {
            try
            {
                using (var db = new pf_course_manageContext())
        ...
        /// </summary>
        /// <returns></returns>
        public object GetAvgLearningTime()
        {
            try
            {
                using (var db = new pf_course_manageContext())
        ...
        /// </summary>
        /// <returns></returns>
        public object GetCourseFinishRate(bool IsAsc, string FeildName)
        {
            try
            {
                using (var db = new pf_course_manageContext())
                {
                    ...
                    //            where r.delete_flag == 0 && r.dif == "1" && e.delete_flag == 0
                    //            select new { c.id, c.course_name, e.learning_progress };
                    //List<CourseFinishRate> courseFinishProgress = new List<CourseFinishRate>();
                    //foreach (var item in query)
                    //{
                    //    courseFinishProgress.Add(new CourseFinishRate()
                    //    {
                    //        CourseID = item.id,
                    //        CourseName = item.course_name,
                    //        LearnProgress = decimal.Parse(item.learning_progress)
                    //    });
                    //}
                    ////分组累加
                    //var queryGroup = courseFinishProgress.GroupBy(x => new { x.CourseID, x.CourseName }).Select(a => new { a.Key.CourseID, a.Key.CourseName, sumProgress = a.Sum(x => x.LearnProgress) });
                    ////完成率
                    //List<CourseFinishRate> courseFinishRates = new List<CourseFinishRate>();
                    //foreach (var item in queryGroup)
                    //{
                    //    var queryStuCount = from c in db.t_plan_course_task_ref
                    //                        join s in db.t_trainingplan_stu on c.plan_id equals s.trainingplan_id
                    //                        where c.delete_flag == 0 && s.delete_flag == 0 && c.content_id == item.CourseID
                    //                        select new { s.id, s.trainingplan_id, s.user_name };

                    //    courseFinishRates.Add(new CourseFinishRate()
                    //    {
                    //        CourseID = item.CourseID,
                    //        CourseName = item.CourseName,
                    //        Rate = (item.sumProgress / queryStuCount.Count()).ToString("#0.00")
                    //    });
                    //} 
                    #endregion
                    List<CourseFinishRate> courseFinishRates = new List<CourseFinishRate>();
                    var queryCourseFinishRate = from c in db.t_course
                                                join r in db.t_plan_course_task_ref on c.id equals r.content_id
        ...
        /// </summary>
        /// <returns></returns>
        public object GetKnowledgeDegree()
        {
            try
            {
                using (var db = new pf_course_manageContext())
