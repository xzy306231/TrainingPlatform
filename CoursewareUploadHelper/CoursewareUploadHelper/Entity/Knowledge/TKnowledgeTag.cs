//------------------------------------------------------------------------------
// <auto-generated>
//     此代码由工具生成。
//     运行时版本:4.0.30319.42000
//     Website: http://www.freesql.net
//     对此文件的更改可能会导致不正确的行为，并且如果
//     重新生成代码，这些更改将会丢失。
// </auto-generated>
//------------------------------------------------------------------------------
using System;
using System.Collections.Generic;
using Newtonsoft.Json;
using FreeSql.DataAnnotations;

namespace CoursewareUploadHelper.Entity.Knowledge {

	[JsonObject(MemberSerialization.OptIn), Table(Name = "t_knowledge_tag")]
	public partial class TKnowledgeTag {

		[JsonProperty, Column(Name = "create_by")]
		public long? CreateBy { get; set; }

		[JsonProperty, Column(Name = "create_time")]
		public DateTime? CreateTime { get; set; }

		[JsonProperty, Column(Name = "delete_flag")]
		public short DeleteFlag { get; set; }

		[JsonProperty, Column(Name = "id", IsIdentity = true)]
		public long Id { get; set; }

		[JsonProperty, Column(Name = "parent_id")]
		public long? ParentId { get; set; }

		[JsonProperty, Column(Name = "tag", DbType = "varchar(50)")]
		public string Tag { get; set; }

		[JsonProperty, Column(Name = "tag_desc", DbType = "text")]
		public string TagDesc { get; set; }

		[JsonProperty, Column(Name = "tag_sort")]
		public int? TagSort { get; set; }

		[JsonProperty, Column(Name = "update_by")]
		public long? UpdateBy { get; set; }

		[JsonProperty, Column(Name = "update_time")]
		public DateTime? UpdateTime { get; set; }


		#region 导航属性

	    public TKnowledgeTag Parent { get; set; }

	    public ICollection<TKnowledgeTag> Tags { get; set; }
		#endregion
	}

}
