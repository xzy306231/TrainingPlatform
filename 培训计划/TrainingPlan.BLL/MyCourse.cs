...
{
    public object GetMyTrainingPlanLList(pf_training_plan_v1Context db, string UserID)
    {
        try
        {
            ...
            var query = from s in db.t_trainingplan_stu
                        ...
                        where s.delete_flag == 0 && p.delete_flag == 0 && s.user_number == UserID
    ...
    public object GetMyCourse(pf_training_plan_v1Context db, IConfiguration configuration, long PlanID, string startTime, string endTime, string strKeyWord, string UserID, int pageSize, int pageIndex)
    {
        try
        {
            string strFds = configuration["FastDFSUrl"];
    ...
    public object GetStuCourseLearningRecord(pf_training_plan_v1Context db, string UserID, long PlanID, long CourseID)
    {
        try
        {
            ...
            else
            {
                ...
                var queryStatus = from s in db.t_course_node_learning_status
                                  ...
                                  select new { s.course_id, s.course_struct_id };
                var qs = queryStatus.FirstOrDefault();
                if (qs != null)
                {
                    var quer = (from s in db.t_course_struct
                                where s.course_id == qs.course_id && s.id == qs.course_struct_id
                ...
                var query = from s in db.t_course_struct
                            ...
                            on new { ID = s.id, CourseID = s.course_id } equals new { ID = p.course_struct_id, CourseID = p.course_id } into sp
                ...
                if (queryList.Count > 0)
                {
                    for (int i = 0; i < queryList.Count; i++)
                    {
                        ...
                        GetChildCourseStruct(db, node, CourseID, RecordID);
                        list.Add(node);
                    }
                }
    ...
    private void GetChildCourseStruct(pf_training_plan_v1Context db, CourseStructRecord courseStruct, long CourseID, long RecordID)
    {
        try
        {
            ...
            if (queryList.Count > 0)
            {
                for (int i = 0; i < queryList.Count; i++)
                {
                    ...
                    node.AttemptNum = queryList[i].attempt_number;
                    node.LastLearnTime = queryList[i].last_learning_time;
                    GetChildCourseStruct(db, node, CourseID, RecordID);//递归
    ...
    public object LearningContinue(pf_training_plan_v1Context db, long RecordID)
    {
        try
        {
