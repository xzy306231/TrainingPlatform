...
using System.Text;
    ...
    {
        public object GetMyTrainTaskPlanList(pf_training_plan_v1Context db,string UserID)
        {
            try
            {
                Dictionary<long, string> dic = new Dictionary<long, string>();
                    ...
                    if (queryList.Count > 0)
                    {
                        ...
                        {
                            var q = from c in db.t_plan_course_task_exam_ref
        ...
        public object GetMyTrainingTask(pf_training_plan_v1Context db,long PlanID, string startTime, string endTime, string strKeyWord, string UserID, int pageSize, int pageIndex)
        {
            try
            {
                    ...
                    foreach (var item in query)
                    {
                        var queryTask = (from p in db.t_plan_course_task_exam_ref
                                         ...
                                         select new { ContentID = p.id, ID = t.id, SrcID = t.original_id, t.task_name, t.task_desc }).ToList();
