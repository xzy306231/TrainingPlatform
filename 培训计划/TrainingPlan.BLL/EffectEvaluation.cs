...
{
    public object GetEndTrainingPlan(pf_training_plan_v1Context db, string keyWord, string startTime, string endTime, int pageIndex, int pageSize)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(startTime) && !string.IsNullOrEmpty(endTime))
            {
                query = db.t_training_plan.Where(x => x.delete_flag == 0
                                ...
                                && (DateTime.Parse(startTime) <= x.start_time)
                                && (DateTime.Parse(endTime) >= x.end_time));
            }
    ...
    #region 个人报告
    public object GetPersonalPlanInfo(pf_training_plan_v1Context db, string userNumber, long planId)
    {
        try
        {
    ...
    public object GetStuByPlanID(pf_training_plan_v1Context db, long planId)
    {
        try
        {
            var queryStu = from s in db.t_trainingplan_stu
                           where s.delete_flag == 0 && s.trainingplan_id == planId
                           select new { s.user_number, s.user_id, s.user_name };
    ...
    public object GetPersonalTheoryReportResult(pf_training_plan_v1Context db, string userNumber, long planId)
    {
        try
        {
            ReportResult reportResult = new ReportResult();
            ...
            //培训计划下课程数量
            var planCourseCount = db.t_plan_course_task_exam_ref.Where(x => x.delete_flag == 0 && x.dif == "1").Count();

            #region 学习时长指标

            var queryPlanLearningTime = from p in db.t_training_plan
                                        join a in db.t_plan_course_task_exam_ref on p.id equals a.plan_id
                                        ...
                                        select new { r.user_number, learningtime = r.learning_sum_time };
            ...
            {
                PlanLearningTimeGroup.Add(new PlanStuLearningTime() { user_id = item.user_number, learningHours = (double)item.learningtime / 3600 });
            }
            ...
            var queryPlanFinishRate = from p in db.t_training_plan
                                      ...
                                      join r in db.t_learning_record on a.id equals r.content_id
                                      where p.id == planId && a.dif == "1" && p.delete_flag == 0 && a.delete_flag == 0 && r.delete_flag == 0 && r.learning_progress != null
                                      select new { r.user_number, r.learning_progress };
            //分组降序
            List<TheoryFinishRate> listPlanFinishRate = new List<TheoryFinishRate>();
            foreach (var item in queryPlanFinishRate)
            {
                listPlanFinishRate.Add(new TheoryFinishRate { UserNumber = item.user_number, Progress = double.Parse(item.learning_progress) });
            }
    ...
    public void GetPersonalExamReportResult(ReportResult reportResult, string userNumber, long planId, IHttpClientHelper client, IConfiguration configuration)
    {
        try
        {
            string url = configuration["ReportResult"];
    ...
    #region 整体报告
    public object GetPlanReportResult(pf_training_plan_v1Context db, long planId)
    {
        try
        {
            var queryPlanF = db.t_training_plan.Where(x => x.delete_flag == 0 && x.id == planId).FirstOrDefault();
    ...
    public object GetPlanTheoryReportResult(pf_training_plan_v1Context db, long planId)
    {
        try
        {
            ...
            //本次培训计划下课程的数量
            var queryTheoryCourseCount = db.t_plan_course_task_exam_ref.Where(x => x.delete_flag == 0 && x.plan_id == planId && x.dif == "1").Count();

            //所有培训计划下课程数量
            var queryPlanCourseCount = db.t_plan_course_task_exam_ref.Where(x => x.delete_flag == 0 && x.dif == "1").Count();
            if (queryTheoryCourseCount == 0)//本次计划下不存在课程
            {
                return new
                {
                    code = 200,
            ...
            var queryPlanFinishRate = (from p in db.t_training_plan
                                      ...
                                      join r in db.t_learning_record on a.id equals r.content_id
                                      where a.dif == "1" && p.delete_flag == 0 && a.delete_flag == 0 && r.delete_flag == 0
                                      select new { p.id, r.learning_progress }).ToList();
            List<TheoryFinishRate> listPlanFinishRate = new List<TheoryFinishRate>();
            foreach (var item in queryPlanFinishRate)
            {
                double temp = 0;
    ...
    public object GetPersonnalReport(pf_training_plan_v1Context db, long userId, string userNumber, long planId, IHttpClientHelper client)
    {
        try
        {
            var queryPlanF = db.t_training_plan.Where(x => x.delete_flag == 0 && x.id == planId).FirstOrDefault();
            var planInfo = GetPersonalPlanInfo(db, userNumber, planId);
            ...
            {
                theoryInfo = GetPersonalTheoryReportResult(db, userNumber, planId);
            ...
            try
            {
                ...
                {
                    var examUrl = "http://EXAMINATION-SERVICE/examination/v1/GetPersonalExamReportResult?";
            ...
            try
            {
                ...
                {
                    var taskUrl = "http://TASKMANAGE-SERVICE/taskManage/v1/PersonalReportInfo?";
            ...
            try
            {
                var endCommentUrl = "http://DATASTATISTIC-SERVICE/statistic/v1/GetEndCommment?";
    ...
    public object GetPlanReport(pf_training_plan_v1Context db, long planId, IHttpClientHelper client)
    {
        try
        {
            var queryPlanF = db.t_training_plan.Where(x => x.delete_flag == 0 && x.id == planId).FirstOrDefault();
            var planInfo = GetPlanReportResult(db, planId);

            string theoryResult = "1", examResult = "1", taskResult = "1";
            object theoryInfo = null, examInfo = null, taskInfo = null, endComment = null;
            if (queryPlanF.course_flag == 1)
            {
                theoryInfo = GetPlanTheoryReportResult(db, planId);
                string str = Newtonsoft.Json.JsonConvert.SerializeObject(theoryInfo);
                var obj = Newtonsoft.Json.JsonConvert.DeserializeObject<ReponseInfomation>(str);
                theoryResult = obj.result.TheoryGlobalResult;
            }
            try
            {
                if (queryPlanF.exam_flag == 1)
                {
                    var examUrl = "http://EXAMINATION-SERVICE/examination/v1/GetPlanExamReportResult?";
            ...
            try
            {
                if (queryPlanF.task_flag == 1)
                {
                    var taskUrl = "http://TASKMANAGE-SERVICE/taskManage/v1/CollectiveReportInfo?";
